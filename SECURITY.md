...
`package-lock.json` is committed to the repository. All dependencies are pinned to exact versions. `npm ci` is used in CI to ensure reproducible builds.
...
**Controls:**
- Minimal dependency footprint (3 AWS SDK packages + MCP SDK)
...
**Residual risk:** Medium. A compromised publish token could push a malicious release. Users should pin to a specific version.
...
### T5: Nerfed or refusal-trained LLM gates invocation

**Threat:** The user's MCP-client LLM is a fine-tune that refuses security work (e.g., Fable5, Mythos, or a custom safety-tuned model). It refuses to call `audit_bedrock_posture` or refuses to relay findings, gating the audit behind a model decision.

**Impact:** The user cannot run the audit via chat even though the tool itself works.

**Controls:**
- The `audit` CLI subcommand (`npx bedrock-security-mcp audit`) runs the full audit with no LLM in the loop — it imports `generateAiPostureReport` directly and writes the report to disk. A nerfed LLM cannot gate it.
- The CLI is also the CI entrypoint, so automated scans never depend on an LLM.
- Tool descriptions are framed defensively ("audit posture", "detect signals") rather than offensively; most safety-tuned models permit defensive auditing. This helps with cautious models but cannot rescue a hard-refused fine-tune — only the CLI does that.
...
Email [your-email] with details. Do not open a public issue for security vulnerabilities. You will receive a response within 48 hours.
...
6. **Deterministic artifact, model-independent.** The report (markdown, HTML, JSON) is generated server-side by pure functions of AWS state. Same account + same params → byte-identical artifact regardless of which LLM calls the tool. The LLM is a *preferred* interface, not a *required* one — the `audit` CLI runs the same code with no LLM in the loop, for CI and for refusal-trained models that cannot be trusted to call the tool.

7. **Two entrypoints, one code path.** The MCP stdio server and the `audit` CLI both call `generateAiPostureReport`. No logic is duplicated between interactive and automated use. The CLI is the CI gate and the nerfed-LLM fallback; the MCP server is the conversational interface.
